...
export function putProductionTerminationDetail(data, successCallback, failedCallback) {
    return dispatch => {
        // Change the global state to attempting to log in.
        store.dispatch(
            setProductionDetailRequest()
        ...
        // Perform our API submission.
        customAxios.put(MIKAPONICS_PRODUCTION_TERMINATION_API_URL+data.slug, buffer).then( (successResponse) => {
            // Decode our MessagePack (Buffer) into JS Object.
            const responseData = msgpack.decode(Buffer(successResponse.data));
            let device = camelizeKeys(responseData);
